...
public class SeckillServiceImpl implements SeckillService {
    ...
    /**
     * @Autowired 和 @Resource注入的区别在于，一个是按类型注入，一个是按名字注入，
     * 推荐按类型注入，因为我们在进行包扫描时，扫描的是类名
     */
    @Autowired
    ...
    //md5盐值字符串，用于混淆md5
    private final String slat = "sadkfjalsdjfalksj23423^&*^&!EBJKH#efadaf4";
    ...
    public Exposure exportSeckillUrl(long seckillId) {
        ...
        if(seckill == null) {
            ...
            if (seckill == null)
                return new Exposure(seckillId, false);
            else {
                //3.放入redis
        ...
        if (nowTime.getTime() < startTime.getTime() || nowTime.getTime() > endTime.getTime())
            return new Exposure(seckillId, false, nowTime.getTime(), startTime.getTime(), endTime.getTime());
        //转定特定字符串的过程，不可逆
